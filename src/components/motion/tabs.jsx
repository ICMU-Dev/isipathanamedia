...
export function TabsContent({ value, children, className, forceMount }) {
  ...
  const reduce = useReducedMotion();
  const active = current === value;
  // Inactive panels are unmounted by default to improve performance, especially
  // on mobile, by preventing massive hidden DOM trees. Use `forceMount={true}`
  // if you strictly need SEO or background persistence.
