...
function buildBoxShadow(glowColor, intensity) {
  ...
  return layers
    .map(([x, y, blur, spread, alpha, inset]) => {
      const currentAlpha = Math.min(alpha * intensity, 100);
      return `${inset ? "inset " : ""}${x}px ${y}px ${blur}px ${spread}px hsl(${base} / ${currentAlpha}%)`;
    })
    .join(", ");
}
...
const BorderGlow = ({
  ...
  const handlePointerMove = useCallback(
    (e) => {
      ...
      setEdgeProximity(getEdgeProximity(card, mouseX, mouseY));
      setCursorAngle(getCursorAngle(card, mouseX, mouseY));
    },
  ...
  return (
    <div
      ...
      className={`relative grid isolate border border-white/[0.06] 5 ${className}`}
      style={{
